    ...
    <div v-for="(value, key, index) in shares" :key="index">
      <div class="message">
        <div class="flex">
          ...
          <img src="../assets/heart.png" class="icon" @click="fav(key)" alt="">
          <p class="number">{{ value.like.length }}</p>
          <img src="../assets/cross.png" class="icon" alt="" @click="del(key)" v-if="path && profile" />
          <img src="../assets/detail.png" class="icon detail" alt="" @click="$router.push({
...
export default {
  ...
  methods: {
      ...
      if (result) {
        this.share[index].like.forEach((element) => {
          if (element.user_id == this.$store.state.user.id) {
            axios({
              method: "delete",
              url: "https://warm-ridge-51366.herokuapp.com/api/like",
              data: {
        ...
        axios
          .post("https://warm-ridge-51366.herokuapp.com/api/like", {
            share_id: this.shares[index].item.id,
            user_id: this.$store.state.user.id,
          })
          .then((response) => {
    ...
    del(index) {
      axios
        .delete(
          "https://warm-ridge-51366.herokuapp.com/api/shares/" +
